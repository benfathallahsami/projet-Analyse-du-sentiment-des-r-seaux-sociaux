...
object SentimentTrainer {
  def main(args: Array[String]) {
    ...
    header.createCell(1).setCellValue("Texte prétraité")
    header.createCell(2).setCellValue("Probabilité Positif")
    header.createCell(3).setCellValue("Probabilité Négatif")
    header.createCell(4).setCellValue("Prédiction")

    // Writing predictions to the Excel sheet
    predictions.zipWithIndex.foreach { case (row, index) =>
      ...
      val predictionLabel = if (prediction == 1.0) "Positif" else "Négatif"

      // Creating a new row in the Excel sheet
      val excelRow = sheet.createRow(index + 1)
      excelRow.createCell(0).setCellValue(itemID)
      excelRow.createCell(1).setCellValue(preprocessed)
      excelRow.createCell(2).setCellValue(formattedProbability(1))
      excelRow.createCell(3).setCellValue(formattedProbability(0))
      excelRow.createCell(4).setCellValue(predictionLabel)
    }

    // Writing the Excel file to disk
    val fileOut = new FileOutputStream("predictions.xlsx")
